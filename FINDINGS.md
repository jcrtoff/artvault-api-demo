...
### F-04 — no `GET /artists/{id}`, and the artist name is missing from the artworks envelope
- **Severity:** friction
- **Where:** `GET /artists/{id}/artworks` (and the absent `GET /artists/{id}`)
- **What we tried:** Render a header (artist name) on the `/artists/{id}` page that lists an artist's artworks across collections.
- **What hurt / what we expected:** There's no `GET /artists/{id}` to fetch a single artist's metadata, and the `GET /artists/{id}/artworks` envelope doesn't carry the artist `name` at the top level — only inside each item's optional `field_values.artist_name`. So the one screen that needs the name is the one place it may be absent (an artist with zero artworks, or whose field group omits `artist_name`, yields nothing), even though the backend knows the name (the `id` is a hash of it). We had to scrape it from the first item's `field_values.artist_name` and fall back to "Artist". Want: `GET /artists/{id}` returning the full artist object, and/or a top-level `name` on the artworks envelope. Minor (docs-only): `GET /collections/{id}/artists` items omit `primary_image_url`/`collections[]` (can't reuse one Artist type); and the name-hash `id` changes on rename, breaking bookmarked URLs.
- **AV ticket:** AV-227
- **Status:** filed
